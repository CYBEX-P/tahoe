...
# ==== Exceptions ====
class DependencyError(Exception): # raised when deleting an attribute referred by other instances
  pass

# ==== Globals ====
LIM = 10
P = {"_id" : 0} # default projection for mongo query

# ==== Class ====

class Instance():
  """
  Use set_class_backend() to change backend;
  If default class backend is not set, each instance will
  make one connection to MongoDB, overwhelming it.
  """
  try: backend = _BACKEND
  except: backend = NoBackend()
  
  def __init__(self, **kwargs):
    if 'backend' in kwargs:   # To override default class backend
...
##  self.validate()
  ...
  def branches(self): # move to report.py
  ...
  def parents(self, q={}, p={"_id":0}): # A(O,E,R) O(E,R) E(S,R) S(R) = type(possible parent type)
  ...
  def update(self, update): # do not use, because it does not update _ref or _hash; only used by methods in this file      
...
##  def validate(self): Draft7Validator(schema[self.itype]).validate(self.doc())
  ...
  def verified_instances(self, instances, type_list): # checks if supplied instances are valid
...
class Event(OES):
  ...
  def unique(self): self.itype + self.sub_type + self.orgid + self.timestamp +  canonical(self.data)
...
class Object(OES):
  ...
  def add_instance(self, data, update=False):
    ...
    self._cref = list( set( self._cref + c_ref )  )
            
    new_hash = hashlib.sha256(self.unique()).hexdigest()
    if new_hash != self._hash:
      self._hash = new_hash
      self.deduplicate
    else:
      self.update({"data" : self.data, "_ref": self._ref, "_cref":self._cref, '_hash':self._hash})
  
  def remove_instance(self, data):  
    data = self.verified_instances(data, [Attribute, Object])
    
    all_ref = set()
    def get_all_ref(uuid):
      all_ref.add(uuid)
      c_ref = self.backend.find_one( {'uuid':uuid}, {**P, 'uuid':1, '_ref':1}).pop('_ref', [])
      for u in c_ref: get_all_ref(u)
      
    for inst in data:
      if inst.uuid not in self._ref: raise KeyError(inst.uuid)
      get_all_ref(inst.uuid)
    
    self._ref = list( set(self._ref) - all_ref )
    self._cref = list( set(self._cref) - all_ref )
    
    data = [parse(i) for i in self.backend.find({ 'uuid':{'$in':self._cref} }, P) ]
    self.data = self.makedata(data)
...
class Attribute(Instance):   
  ...
  def delete(self): # needs rigorous testing
    r = self.backend.find_one({"_ref" : self.uuid}, {**P, 'uuid':1})
    if r: raise DependencyError(f"Instance is referred in {r['uuid']}")
  ...
  def relatedsummarybyevent(self, lvl=1, start=0, end=None, limit=LIM, skip=0, page=1):
    r, curpg, nxtpg = self.related(lvl, itype='event', p={"sub_type":1, "data":1}, 
                        start=start, end=end, limit=limit, skip=skip, page=page)
    d = defaultdict(list)
    for i in r: d[i["sub_type"]].append(i["data"]) 
    data = {k:features(v) for k,v in d.items()}
    return data, curpg, nxtpg

  def update(self, update): pass  # updating an attribute is never required  
